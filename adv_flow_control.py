...
def for_ex():
    ...
    for animal in a:
        print(animal, end = " ")
    else: # 반복문이 break되지 않고 정상 종료 되었을 때 마지막에 수행
        print()

    # 반복시 요소와 함께 인덱스도 함께 필요할 때 -> enumerate 함수 -> (인덱스, 요소) 튜플 반환
    for index, color in enumerate(['red', 'blue', 'green', 'black','whiet']): # (인덱스, 요소) 튜플
        print(index, color)
    ...
    for i in r1:
        ...
        print("짝수는 없습니다.") # break를 탈출하면실행되지 않는다
