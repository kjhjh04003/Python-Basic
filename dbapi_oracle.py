...
def create_connection():
    # dsn 작성
    dsn = cx_Oracle.makedsn("localhost", 1521, "xe") # 서버 주소, 포트, 서비스명

    # 접속
    db = cx_Oracle.connect("hr", "hr", dsn) # 계정, 암호, 데이터소스
    return db
...
def test_basic_query():
    ...
    conn = create_connection() # Connection 객체
    cursor = conn.cursor() # Cursor 객체
