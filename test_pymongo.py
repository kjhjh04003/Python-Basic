...
def test_insert_many():
    ...
    # 여러 문서 삽입 시에는 []로 문서를 전달(list)
    xs = coll.insert_many([
        { "name": "고길동", "address": "서울", "method": "insertmany"},
...
def test_update():
    ...
    print(xs.matched_count, "개가 매칭 되었습니다.") # 조건에 맞는문서의 개수
...
if __name__ == "__main__":
    ...
    # test_insert_many()
    # test_update()
    # test_select_by_filter() # 아무인자값이 없으면 filter={}, projection={}
    # test_select_by_filter(projection={"name": 1, "address": 1, "_id": 0})
    # SELECT *FROM 테이블 WHERE 컬럼 LIKE -> 정규식을 이용하여 패턴을 전달(자바스크립트 정규식표현 : /패턴/
    ...
    # address == "서울"인 문서 삭제
    # test_delete_by_filter({ "address": "서울"}) # db.pymongo.delete({"address": "서울"})
    # 전체 문서 삭제
    # db.pymongo.delete({})
