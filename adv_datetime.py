...
def get_datetime():
    ...
    # 요일 확인: weekday() 메서드, 월:0 ~ 일:6
    print("오늘은 무슨 요일인가?", now.weekday())

    # datetime 객체는 date 객체 + time 객체가 함꼐 있는 것
    print(now.date(), type(now.date())) # 날자 관련 속성을 가진 date 객체
...
def timedelta_ex():
    ...
    print("COURRENT가 PAST 이후?", current > past)
