...
def rel_oper(): # 비교 연산자 연습
    ...
    print("a가 0보다 크고 a가 10보다 작은가?", a > 0 and a < 10)
    print("a가 0보다 크고 a가 10보다 작은가?", 0 < a and a < 10)
    print("a가 0보다 크고 a가 10보다 작은가?", 0 < a < 10)
...
def variable_ex(): # 변수
    ...
    # 숫자로 시작하면 안된다.
    # 예약어는 사용할 수 없다.
    import keyword # keyword 모듈을 로드한다.
    print(keyword.kwlist)
    print("키워드 갯수 : ", len(keyword.kwlist))

    # 유니코드 이름의 변수도 사용 가능 -> 권장하지 않는다.
    가격 = 12000
    print(가격 + 가격 * 0.1)
...
def assginment_ex(): # 치환문
    ...
    # 여러 변수를 한꺼번에 할당
    a, b = 3.5, 5.3 # 좌변의 변수의 갯수와 우변의 값의 갯수가 일치해야 한다.
    print(a, b)
...
def logical_oper():
    ...
    # 논리합(or : 둘 중 하나만 true이면 true)
    # 논리부정(not : true <-> fale)
    a, b = 20, 30
    print(a < b)
    print(not a < b) # a < b의 논리를 부정
    print(a < b and a != b) # true and true = true
    print(a == b or a != b) # false and true = true
